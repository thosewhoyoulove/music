 ...
 * @LastEditors: 曹俊
 * @LastEditTime: 2022-08-26 11:51:19
-->
<template>
    <div class="w-100% h-100% bg-white">
        <div class="font-700 text-left ml-5">评论({{total}})</div>
        <div class="flex break-all m-1 p-1">
            <img class="h-4.5rem rounded-xl ml-2" :src="state.songDetail[0]?.al?.picUrl" alt="">
            <div class="text-left ml-2">
                <div class="text-style text-sm font-600">{{state.songDetail[0]?.name}}</div>
                <div class="flex items-center" v-for="(item,index) in state.songDetail[0]?.ar" :key="index">
                    <div class="text-xs text-hex-ccc p-1">{{item.name}}</div>
                    <div class="text-xs text-hex-aaa"><van-icon color="#ccc" name="arrow" /></div>
                </div>
             </div>
             </div>
        <div>
