 ...
 * @LastEditors: 曹俊
 * @LastEditTime: 2022-10-09 21:56:54
-->
<template>
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          ...
          <div class="px-1 text-10px">
            {{ filterTotal(state?.album?.subscribedCount) }}
          </div>
        </div>
      </div>
...
//跳转歌手主页
const toArtistDetail = (state.album.artists[0].id) => {
  console.log(state.album.artists[0].id);
  router.push({
    path: "/Artist",
    query: {
      artistId: state.album.artists[0].id,
    },
  });
};
</script>
